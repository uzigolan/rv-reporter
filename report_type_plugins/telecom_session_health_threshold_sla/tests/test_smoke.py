...
def test_telecom_session_health_threshold_sla_smoke_build() -> None:
    manager = ReportPluginManager(plugin_dir="report_type_plugins")
    result = manager.compute_metrics(
        metrics_profile="telecom_session_health_threshold_sla",
        df=pd.DataFrame({"sample": [1, 2, 3]}),
        prefs={},
        report_type_id="telecom_session_health_threshold_sla",
    )
